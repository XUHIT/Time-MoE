...
def evaluate(args):
    ...
    if rank == 0:
        #只让 主进程，第一张显卡（rank 0） 执行以下代码，比如打印、保存、写文件。
        item = {
            'model': args.model,
            'data': args.data,
            'context_length': args.context_length,
        ...
        # 以追加模式写入到 results/eval_results.txt，每行为一次预测记录
        os.makedirs('results', exist_ok=True)
        result_line = json.dumps(item)
        with open('results/eval_results.txt', 'a') as f:
            f.write(result_line + '\n')
        print(f">>> Appended result to results/eval_results.txt")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model', '-m',
        type=str,
        default='pretrained_model/TimeMoE-200M',
